...
/** クイズ画面用コントローラ */
app.controller('main', ['$scope', '$q', 'fileResource', 'qCommon', 'round', 'serverResource',
	function ($scope, $q, fileResource, qCommon, round, serverResource) {
		/* ブラウザモード判定 */
		$scope.browserMode = qCommon.browserMode();

		var fs;
		if (!$scope.browserMode) {
			fs = require('fs');
		}
		...
		/* getTextImageH - テキストイメージ（横書き）を返却する */
		$scope.getTextImageH = qCommon.getTextImageH;
		/* calc - 再計算関数 */
		$scope.calc = function () {
		...
		console.log("browser mode", qCommon.browserMode())
		if (qCommon.browserMode()) {
			fr.push(serverResource.get().$promise);
		}

		$q.all(fr).then(function (strs) {
			// 初期アニメーション停止
			...
			if (!qCommon.viewMode()) {
				...
				console.log("ブラウザモード時途中経過から");
				if (qCommon.browserMode()) {
					qCommon.downloadStatus($scope);
				}
			}
			console.log("履歴初期化");
			// 履歴
			...
			// 履歴が無い場合、現在の得点状況を追加
			if (!qCommon.viewMode() && !qCommon.browserMode()) {
				console.log("履歴作成");
...
app.factory('serverResource', function ($resource, qCommon) {
	return $resource('https://a2v4fab6o7.execute-api.ap-northeast-1.amazonaws.com/default/union-get?id=entry&round=' + qCommon.getRoundName());
});
